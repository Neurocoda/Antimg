...
// ProcessImage 处理上传的图片 - 移除了大小和尺寸限制
func (s *ImageService) ProcessImage(src io.Reader, attackLevel float64) (image.Image, string, error) {
	...
	// 返回处理后的图片和原始格式信息
	return processedImg, format, nil
}
...
func (s *ImageService) applyAggressiveNoiseAttack(img image.Image, level float64) image.Image {
	...
	// 强力亮度攻击
	brightnessChange := (rand.Float64() - 0.5) * level * 60 // 最大±30亮度变化
	result = imaging.AdjustBrightness(result, brightnessChange)

	// 强力对比度攻击
	contrastChange := (rand.Float64() - 0.5) * level * 80 // 最大±40对比度变化
	result = imaging.AdjustContrast(result, contrastChange)

	// 多次随机调整
	rounds := int(level*3) + 1
	for i := 0; i < rounds; i++ {
		brightness := (rand.Float64() - 0.5) * level * 20
		contrast := (rand.Float64() - 0.5) * level * 30
...
func (s *ImageService) applyAggressiveGeometricAttack(img image.Image, level float64) image.Image {
	...
	// 强力旋转攻击
	if level > 0.2 {
		angle := (rand.Float64() - 0.5) * level * 15 // 大幅增加旋转角度
		result = imaging.Rotate(result, angle, color.Transparent)
	}

	// 强力缩放攻击
	if level > 0.3 {
		scaleFactor := 1.0 + (rand.Float64()-0.5)*level*0.2 // 大幅增加缩放范围
	...
	for i := 0; i < rounds; i++ {
		// 随机旋转
		angle := (rand.Float64() - 0.5) * level * 8
		result = imaging.Rotate(result, angle, color.Transparent)

		// 随机缩放
		scale := 1.0 + (rand.Float64()-0.5)*level*0.1
	...
	// 最终强力变换
	if level > 0.8 {
		finalAngle := (rand.Float64() - 0.5) * level * 20
...
func (s *ImageService) applyAggressiveColorAttack(img image.Image, level float64) image.Image {
	...
	for i := 0; i < rounds; i++ {
		brightnessChange := (rand.Float64() - 0.5) * level * 50 // 大幅增加亮度变化
		contrastChange := (rand.Float64() - 0.5) * level * 60   // 大幅增加对比度变化
		result = imaging.AdjustBrightness(result, brightnessChange)
		result = imaging.AdjustContrast(result, contrastChange)
	}
...
func (s *ImageService) applyFinalMixedAttack(img image.Image, level float64) image.Image {
	...
	for i := 0; i < 3; i++ {
		...
		// 强力亮度对比度调整
		brightness := (rand.Float64() - 0.5) * level * 40
		contrast := (rand.Float64() - 0.5) * level * 50
		result = imaging.AdjustBrightness(result, brightness)
		result = imaging.AdjustContrast(result, contrast)

		// 旋转攻击
		angle := (rand.Float64() - 0.5) * level * 10
		result = imaging.Rotate(result, angle, color.Transparent)
