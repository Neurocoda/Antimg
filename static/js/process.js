...
// 初始化客户端处理器
function initializeClientProcessor() {
    clientProcessor = new ClientImageProcessor();
...
function processImageWithPreview() {
    ...
    const attackLevel = parseFloat(document.getElementById('attackLevel').value);
    ...
    if (currentAttackValue && attackLevelInfo) {
        currentAttackValue.textContent = attackLevel.toFixed(2);
    ...
    // 客户端处理图像
    processImageLocally(attackLevel)
        .then(blob => {
            // 创建处理后图片的URL
...
// 客户端本地图像处理
async function processImageLocally(attackLevel) {
    try {
        // 加载图像到处理器
        await clientProcessor.loadImage(currentImageFile);
        
        // 模拟处理进度
        simulateProcessingProgress();
        
        // 执行图像处理
        const processedBlob = await clientProcessor.processImage(attackLevel);
        
        return processedBlob;
    } catch (error) {
        console.error('客户端处理失败:', error);
        throw error;
    }
}
...
function simulateProcessingProgress() {
    ...
    const progressInterval = setInterval(() => {
        progress += Math.random() * 10 + 5; // 随机增长5-15%
        if (progress > 90) {
...
// 启动进度条动画（已被simulateProcessingProgress替代）
function startProgressAnimation() {
    // 这个函数现在由simulateProcessingProgress处理
    // 保留以兼容现有代码
}
